-- Migration: Event-Driven Materialized Badge Progress System with Tuple-Based Criteria

-- Create the user_badge_progress table to store precalculated progress.
...
create policy "Allow public read access to user_badge_progress" on public.user_badge_progress
  for select using (true);

create policy "Users can update their own badge pinning on user_badge_progress" on public.user_badge_progress
...
begin
  ...
  -- Maintain user_badges table compatibility for unlocked badges
  if v_all_criteria_met then
    insert into public.user_badges (user_id, badge_id, acquired_at, is_pinned)
    values (p_user_id, p_badge.id, v_existing_acquired_at, coalesce(v_existing_is_pinned, false))
    on conflict (user_id, badge_id) do update set
      acquired_at = coalesce(public.user_badges.acquired_at, excluded.acquired_at);
  end if;
end;
$$;
...
-- Trigger on location_visits to evaluate affected badges incrementally.
